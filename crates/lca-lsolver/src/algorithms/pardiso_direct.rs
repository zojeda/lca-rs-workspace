...
impl Default for PardisoConfig {
    fn default() -> Self {
        Self {
            matrix_type: PardisoMatrixType::General,
            max_threads: None,
        }
    }
}
...
impl PardisoDirect {
    pub fn new(config: PardisoConfig) -> Self {
        Self { config }
    }
}
...
impl SolveAlgorithm<CpuDevice, SparseMatrix> for PardisoDirect {
    ...
    > + Send {
        use pardiso_wrapper::{MKLPardisoSolver, MatrixType, PardisoInterface};
        ...
        async move {
            ...
            let mut solver = MKLPardisoSolver::new()
                .map_err(|e| LcaCoreError::LinkError(format!("PARDISO init error: {e}")))?;
            solver.set_matrix_type(mtype);
            solver
                .pardisoinit()
                .map_err(|e| LcaCoreError::LinkError(format!("PARDISO init error: {e}")))?;
            if let Some(t) = config.max_threads {
                let _ = solver.set_num_threads(t as i32);
            }

            // 1) Analysis
            solver.set_phase(pardiso_wrapper::Phase::Analysis);
            solver
                .pardiso(&a_vals, &ia, &ja, &mut [], &mut [], n as i32, 1)
            ...
            solver
                ...
                .map_err(|e| {
                    LcaCoreError::LinkError(format!("PARDISO factorization error: {e}"))
                })?;
            ...
            solver
                .pardiso(&a_vals, &ia, &ja, &mut rhs, &mut x, n as i32, 1)
                .map_err(|e| LcaCoreError::LinkError(format!("PARDISO solve error: {e}")))?;

            Ok(SolveResult {
                x,
                metadata: PardisoMetadata {
                    n,
                    nnz: a.nnz(),
                    iterations: None,
                },
...
fn csr_usize_to_csr_i32_1based(
    a: &SparseMatrix,
) -> Result<(Vec<i32>, Vec<i32>, Vec<f64>), LcaCoreError> {
    let n = a.rows();
    if a.cols() != n {
    ...
    for rp in a.row_ptr() {
        let v = i32::try_from(*rp)
            .map_err(|_| LcaCoreError::InvalidDimensions("row_ptr index overflow".into()))?;
        ia.push(v + 1);
    }
    let mut ja = Vec::with_capacity(nnz);
    for ci in a.col_indices() {
        let v = i32::try_from(*ci)
            .map_err(|_| LcaCoreError::InvalidDimensions("col index overflow".into()))?;
        ja.push(v + 1);
    }
