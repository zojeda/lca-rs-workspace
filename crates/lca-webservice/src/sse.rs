...
use std::fmt;
use tokio::sync::mpsc;
use tracing::{Event as TracingEvent, Level, Subscriber, field::Visit};
use tracing_subscriber::{Layer, layer::Context}; // Removed FilterExt
...
impl<'a> Visit for SseTracingVisitor<'a> {
    fn record_debug(&mut self, field: &tracing::field::Field, value: &dyn fmt::Debug) {
        ...
        if field_name == "message" {
            // Standard field for log messages via log::* macros
    ...
    fn record_str(&mut self, field: &tracing::field::Field, value: &str) {
        ...
        if field_name == "message" {
            ...
            self.fields
                .insert(field_name.to_string(), Value::String(value.to_string()));
        }
    }
}
...
impl<'a> SseTracingVisitor<'a> {
    ...
    fn finish_event(self) {
        ...
        if !(self.target.starts_with("lca_rs")
            ...
            || self.target.starts_with("lca_webservice::sse"))
        {
            // Allow logs from sse module itself for status
            return;
        }
        ...
        let progress_update = ProgressUpdate {
            ...
            details: if self.fields.is_empty() {
                None
            } else {
...
impl<S: Subscriber + Send + Sync + for<'lookup> tracing_subscriber::registry::LookupSpan<'lookup>>
    Layer<S> for SseTracingLayer
{
    fn on_event(&self, event: &TracingEvent<'_>, _ctx: Context<'_, S>) {
        ...
        if *level <= Level::INFO
            || target.starts_with("lca_rs")
            || target.starts_with("lca_core")
            || target.starts_with("lca_webservice::handler")
        {
