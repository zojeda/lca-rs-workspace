use lca_rs::core::models::DemandItem;
use serde::{Deserialize, Serialize};
// Added for json! macro in schema example
use utoipa::ToSchema;
...
pub struct ProcessRequest {
    ...
    pub name: String,
    #[validate(length(min = 1))]
    // Ensures products is not empty. Exact count of 1 is validated by lca-rs.
...
impl From<LcaRequest> for lca_rs::model::LcaModel {
    // LcaModel is in lca_rs::model
    fn from(req: LcaRequest) -> Self {
        lca_rs::model::LcaModel {
            ...
            evaluation_impacts: req
                .evaluation_impacts
                .into_iter()
                .map(|impact_ref_req| impact_ref_req.into())
                .collect(),
        }
    }
}
...
impl From<DemandItemRequest> for DemandItem {
    // DemandItem is pub use lca_rs::DemandItem;
