use crate::{
    ...
    sse::{SseTracingLayer, send_sse_status},
};

use axum_streams::StreamBodyAs;
// Removed: use futures::stream::Stream;
use lca_rs::core::GpuDevice;
use lca_rs::{EvalLCASystem, model::LcaModel};
use serde::Serialize;
...
pub async fn calculate_lca_handler(
    ...
    payload.validate().map_err(AppError::Validation).unwrap();
    ...
    tokio::spawn(async move {
        ...
        // 6. Select device/solver backend and initialize devices as needed
        // Solver is selected at compile time: if the `pardiso` feature is enabled (non-wasm), use CPU PARDISO; otherwise use GPU BiCGSTAB.
        #[cfg(all(feature = "pardiso", not(target_arch = "wasm32")))]
        let use_cpu_pardiso = true;
        #[cfg(not(all(feature = "pardiso", not(target_arch = "wasm32"))))]
        let use_cpu_pardiso = false;

        // Build evaluation system with chosen backend
        let mut eval_system: EvalLCASystem = match lca_system.try_into() {
            Ok(v) => v,
            Err(e) => {
                tracing::error!(target: "lca_webservice::handler", "Failed to build EvalLCASystem: {:?}", e);
                let err_payload = ProgressUpdate {
                    event_type: ProgressUpdateType::Error,
                    message: format!("Failed to build EvalLCASystem: {}", e),
                    details: None,
                    result_data: None,
                };
                if let Ok(json_str) = serde_json::to_string(&err_payload) {
        ...
        // If CPU PARDISO requested, set solver backend accordingly; else we'll use GPU BiCGSTAB.
        #[cfg(all(feature = "pardiso", not(target_arch = "wasm32")))]
        if use_cpu_pardiso {
            // Optional: control MKL/PARDISO threads via env var
            let threads = std::env::var("LCA_SOLVER_THREADS")
                .ok()
                .and_then(|s| s.parse::<usize>().ok());
            tracing::info!(target: "lca_webservice::handler", threads = ?threads, "Using CPU PARDISO backend");
            send_sse_status(
                &task_sse_tx,
                ProgressUpdateType::Status,
                format!(
                    "Using CPU PARDISO backend{}",
                    threads
        ...
        // If GPU path, initialize GPU and pass device into eval_system for consistent tracing/stats
        #[allow(unused_mut)]
        let mut gpu_device: Option<GpuDevice> = None;
        if !use_cpu_pardiso {
            tracing::info!(target: "lca_webservice::handler", "Initializing GPU device (BiCGSTAB solver)...");
            send_sse_status(
                &task_sse_tx,
                ProgressUpdateType::Status,
            ...
            match GpuDevice::new().await {
                Ok(d) => {
                    tracing::info!(target: "lca_webservice::handler", "GPU device initialized successfully (adapter info will be in logs).");
                    send_sse_status(
                        &task_sse_tx,
                        ProgressUpdateType::Status,
                    ...
                    .await;
                    eval_system = eval_system.with_gpu_device(d.clone());
                    gpu_device = Some(d);
                }
                Err(e) => {
            ...
            tracing::info!(target: "lca_webservice::handler", "Skipping GPU initialization (CPU backend selected)");
        }
        ...
        match eval_system.evaluate().await {
            Ok(results) => {
                ...
                // If GPU was used, report transfer stats
                if let Some(dev) = gpu_device.as_ref() {
                    let stats = dev.get_transfer_stats();
                    tracing::info!(target: "lca_webservice::handler", bytes_to_gpu = stats.bytes_to_gpu, bytes_from_gpu = stats.bytes_from_gpu, "GPU transfer stats");
                    let _ = send_sse_status(
                        &task_sse_tx,
                        ProgressUpdateType::Status,
                        format!(
                            "GPU transfer: to_gpu={} bytes, from_gpu={} bytes",
                            stats.bytes_to_gpu, stats.bytes_from_gpu
                        ),
                    )
                    .await;
                } else {
                    #[cfg(all(feature = "pardiso", not(target_arch = "wasm32")))]
                    {
                        let _ = send_sse_status(
                            &task_sse_tx,
                            ProgressUpdateType::Status,
                            "Computation executed on CPU (PARDISO)".to_string(),
                        )
                        .await;
                    }
                }
