...
- Adjust tracing via `RUST_LOG`, e.g. `RUST_LOG=info,lca_webservice=debug,lca_rs=debug`.
 - Solver selection is feature-based:
     - Build with `--features pardiso` to use the CPU direct solver (Intel MKL PARDISO) on non-WASM targets. Optionally set `LCA_SOLVER_THREADS=<n>` to control threads.
     - Without the feature, the GPU BiCGSTAB backend is used by default.