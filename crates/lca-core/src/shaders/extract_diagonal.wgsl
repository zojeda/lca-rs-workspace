...
@group(0) @binding(2) var<storage, read> values: array<f64>;      // CSR values (size NNZ)

// Binding for the output diagonal vector
@group(0) @binding(3) var<storage, read_write> diagonal_output: array<f64>; // Output vector (size N)
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Explicitly declare diagonal_value as f64
    var diagonal_value: f64 = 0.0;
