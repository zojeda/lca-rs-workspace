use crate::{
    context::GpuContext,
    error::LcaCoreError,
    math::{sparse_matrix::SparseMatrixGpu, vector::GpuVector},
};
...
pub(crate) async fn internal_axpy(
    ...
    let shader_module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("Internal AXPY Shader"),
    ...
    let params_buffer = context.create_gpu_buffer_with_data(
        "Internal AXPY Params Buffer",
        bytemuck::bytes_of(&params),
        wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
    );
    let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("Internal AXPY Bind Group Layout"),
        entries: &[
    ...
    let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("Internal AXPY Pipeline Layout"),
        bind_group_layouts: &[&bind_group_layout],
        push_constant_ranges: &[],
    });
    let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: Some("Internal AXPY Pipeline"),
    ...
    let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("Internal AXPY Bind Group"),
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("Internal AXPY Encoder"),
    });
    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("Internal AXPY Compute Pass"),
...
pub(crate) async fn internal_dot(
    ...
    let module_pass1 = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("Internal Dot Product Pass 1 Shader"),
    ...
    let partial_results_buffer = context.create_empty_buffer(
        "Internal Dot Product Partial Results Buffer",
    ...
    let bind_group_pass1 = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("Internal Dot Product Pass 1 Bind Group"),
        layout: &layout_pass1,
        entries: &[
            wgpu::BindGroupEntry {
                binding: 0,
    ...
    let bind_group_pass2 = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("Internal Dot Product Pass 2 Bind Group"),
        layout: &layout_pass2,
        entries: &[
            wgpu::BindGroupEntry {
                binding: 0,
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("Internal Dot Product Encoder"),
    });
    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("Internal Dot Product Pass 1"),
            timestamp_writes: None,
        });
    ...
    if num_workgroups > 1 {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("Internal Dot Product Pass 2"),
            timestamp_writes: None,
        });
    ...
    let result_vec = context
        .read_buffer_to_cpu::<f64>(&final_result_buffer, 1)
        .await?;
...
pub(crate) async fn internal_spmv(
    ...
    let shader_module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("Internal SpMV CSR Shader"),
        source: wgpu::ShaderSource::Wgsl(shader_source.into()),
    });
    ...
    let bind_group_layout_main =
        device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Internal SpMV CSR Main Bind Group Layout"),
            entries: &[
                wgpu::BindGroupLayoutEntry {
                    binding: 0,
                    visibility: wgpu::ShaderStages::COMPUTE,
    ...
    let bind_group_main = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("Internal SpMV CSR Main Bind Group"),
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("Internal SpMV Encoder"),
    });
    let workgroup_size = 256;
    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("Internal SpMV CSR Main Pass"),
            timestamp_writes: None,
        });
...
pub(crate) async fn internal_spmv_transpose(
    ...
    let pipeline_main = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        ...
        module: &shader_module,
        cache: None,
        entry_point: Some("main_spmv_csr_transpose_per_col"),
        compilation_options: Default::default(),
    });
    let bind_group_main = device.create_bind_group(&wgpu::BindGroupDescriptor {
