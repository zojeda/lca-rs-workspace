...
use crate::{
    LcaCoreError, error::Result, math::sparse_matrix::SparseMatrix, math::sparse_matrix::Triplete,
    models::LcaMatrix,
};
...
mod tests {
    ...
    fn test_combine_two_simple_systems_no_links() {
        ...
        assert_eq!(
            ...
            Some(vec![
                DemandItem::new("SysA::Proc0".to_string(), 1.0),
                DemandItem::new("SysB::Proc0".to_string(), 1.0)
            ])
        );
        // Evaluation methods
        assert_eq!(
    ...
    fn test_combine_evaluation_demands_logic() {
        ...
        assert_eq!(
            ...
            vec![
                DemandItem::new("SysA::Proc0".to_string(), 1.0),
                DemandItem::new("SysC::Proc0".to_string(), 1.0)
            ]
        );
    ...
    fn create_test_system(
        ...
        let a_matrix_triplets = a_triplets
            .into_iter()
            .map(|(r, c, v)| Triplete::new(r, c, v))
            .collect();
        ...
        let b_matrix_triplets = b_triplets
            .into_iter()
            .map(|(r, c, v)| Triplete::new(r, c, v))
            .collect();
        ...
        let c_matrix_triplets = c_triplets
            .into_iter()
            .map(|(r, c, v)| Triplete::new(r, c, v))
            .collect();
