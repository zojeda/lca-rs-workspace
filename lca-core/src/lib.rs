...
pub mod error;
#[path = "math/mod.rs"]
pub mod math; // traits, vector, sparse_matrix, ops
#[path = "devices/mod.rs"]
pub mod devices; // cpu/gpu backends
...
pub use models::{LcaMatrix, DemandItem, InterSystemLink, LcaSystem};
...
impl math::sparse_matrix::SparseMatrixGpu {
    pub async fn spmv(&self, x: &math::vector::GpuVector, y: &mut math::vector::GpuVector) -> Result<(), LcaCoreError> {
        math::ops::internal_spmv(&self.context, self, x, y).await
    }
    pub async fn spmv_transpose(&self, x: &math::vector::GpuVector, y: &mut math::vector::GpuVector) -> Result<(), LcaCoreError> {
