...
use lca_database::{
    model::{self, Database, IOType, SemVer}, Error, LcaDatabase, RefRecord, Result, SurrealLcaDb
};
use itertools::Itertools;

// Use structs and Error from lca-database
use std::{cell::Ref, path::PathBuf, str::FromStr as _};
use surrealdb::{
    RecordId, Uuid,
...
use ecospold_parser::{
    masters::{
        classifications::ValidClassificationSystems, elementary_exchanges::ValidElementaryExchanges, geographies::ValidGeographies, intermediate_exchanges::ValidIntermediateExchanges
    },
    model::{ActivityDataset, ElementaryExchange, Exchange as EcoSpoldExchange, Geography},
};
...
async fn import_geographies(
    ...
    for parsed_geography in geographies {
      let location = if let (Some(x), Some(y)) = (parsed_geography.longitude, parsed_geography.latitude) {
        ...
        let parsed_geography_id =
            RecordId::from_table_key("geography", parsed_geography.id);
        ...
        .filter_map(|entry| {
            entry.ok().and_then(|dir_entry| {
                dir_entry.path().extension().and_then(|e| {
                    if e == "spold" {
                        Some(PathBuf::from(dir_entry.path()))
                    } else {
                        None
                    }
                })
        ...
        match ecospold_parser::parse_ecospold(&xml_content) {
            Ok(parsed_data) => {
                ...
                let outputs = parsed_activity
                    ...
                    .filter_map(|f| match f {
                        ecospold_parser::model::Exchange::IntermediateExchange(ie) => {
                          if let Some(IOType::Output(o_type)) = get_io_type(ie.input_group, ie.output_group).ok() {
                            Some(model::ActivityOutput {
                                ...
                                production_volume_comment: ie.production_volume_comment.as_ref().and_then(|c| Some(c.value.clone())),
                                production_volume_mathematical_relation: ie.production_volume_comment.as_ref().and_then(|c| Some(c.value.clone())),
                                amount: ie.amount,
                                output_type: o_type,
                                uom: ie.unit_name.value.clone(),
                                cas_number: ie.cas_number.clone(),
                                comments: Some(ie.comments.iter().map(|c| c.value.clone()).join("\n"))
                            })
                          } else {
                            None
                          }
                ...
                let tech_flows = parsed_activity
                    ...
                    .filter_map(|f| match f {
                        ecospold_parser::model::Exchange::IntermediateExchange(ie) => {
                          if let Some(IOType::Input(i_grp)) = get_io_type(ie.input_group, ie.output_group).ok() {
                            Some(model::TechnosphereFlow {
                                id: RecordId::from_table_key("technosphere_flow", ie.id.clone()),
                ...
                let activity = model::Activity {
                    ...
                    name: parsed_activity
                        ...
                        .value,
                    classifications,
                    geography: RecordId::from_table_key("geography", parsed_activity.activity_description.geography.geography_id),
                    database: database.id.clone(),
                };
                // let json = serde_json::to_string_pretty(&tech_flows.iter().map(|tf| tf.clone()).collect::<Vec<_>>()).unwrap();
                // println!("{}", json);
                db.create_activity(activity, outputs, bio_flows, tech_flows).await.unwrap();
...
fn get_io_type(input_group: Option<i32>, output_group: Option<i32>)->Result<IOType> {
    if let Some(igrp) = input_group {
      ...
      match ogrp {
          ...
          _ => Err(Error::DatabaseImportError("outputGroup invalid".to_string()))
      }
    } else {
      Err(Error::DatabaseImportError("inputGroup or outputGroup required".to_string()))
    }
}